...
def reshuffle_process(data):
    shuffle_data = []
    for item in data:
        new_item = {"question_id": item["question_id"], "question": item["question"]}
        solutions = item["solutions"]
        random.shuffle(solutions)
        sorted_solutions = sorted(solutions, key=lambda x: x['eval_score'], reverse=True)
        eval_list = [solutions.index(item)+1 for item in sorted_solutions]
        new_item["solutions"] = solutions
        new_item["rank_list"] = eval_list
        shuffle_data.append(new_item)
    return shuffle_data
